          ...
          <div style={{ position:'relative', width:'440px', height:'100vh', background:'rgba(6,16,28,0.97)', backdropFilter:'blur(32px)', borderLeft:'1px solid rgba(255,255,255,0.12)', overflowY:'auto', zIndex:1 }}>
            <div style={{ padding:'24px' }}>
              ...
              <div style={{ display:'flex', gap:'7px', flexWrap:'wrap', marginBottom:'20px' }}>
                ...
                <button onClick={()=>{ navigate('/agreements', { state:{ openAdd:true, clientName:selected.name, clientPhone:selected.phone, clientIdNumber:selected.id_number } }) }} style={{ padding:'8px 14px', borderRadius:'9px', fontSize:'11px', fontWeight:600, cursor:'pointer', fontFamily:'inherit', background:'rgba(255,149,0,0.10)', border:'1px solid rgba(255,149,0,0.28)', color:'rgba(255,149,0,0.90)' }}>📋 Rental Agreement</button>
              </div>
                ...
                <div style={{ marginBottom:'16px' }}>
                  ...
                  {clientAgreements.map((a:any) => (
                    <div key={a.id} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'10px 12px', marginBottom:'6px', background:'rgba(255,255,255,0.04)', border:'1px solid rgba(255,255,255,0.08)', borderRadius:'8px' }}>
                      ...
                      <button onClick={()=>{ setSelected(null); setTimeout(()=>{ window.location.href='/agreements?view='+a.id }, 100) }}
